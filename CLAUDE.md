...
Three-stage Docker build (`node:20-alpine`): install deps → `npm run build` (standalone Next.js output, no career-brain access needed at build time since `content/generated/*.json` is already committed; accepts `NEXT_PUBLIC_SITE_URL` as a build arg) → runtime stage runs `node server.js` directly on port 3000 — no nginx in front. `docker-compose.yml` maps `${HOST_PORT:-80}:3000` (host port overridable per environment, defaults to 80), passes the build arg through, and forwards `ANTHROPIC_API_KEY` at runtime (currently unused until Phase 4).
...
- `.github/workflows/deploy.yml` — push to `main` (or manual dispatch) deploys to `/opt/portfolio-web`, port 80 (`HOST_PORT` unset, so the compose default applies), using the `NEXT_PUBLIC_SITE_URL` repo variable.
- `.github/workflows/deploy-dev.yml` — push to `develop` (or manual dispatch) deploys to `/opt/portfolio-web-dev`, port 8080 (`HOST_PORT=8080`), using the `NEXT_PUBLIC_SITE_URL_DEV` repo variable. Because the deploy paths differ, Docker Compose's default project-name-from-directory-basename behavior keeps the two stacks' containers/networks separate on the same box automatically — no explicit `-p`/`COMPOSE_PROJECT_NAME` needed. Port 8080 must be open in the Linode Cloud Firewall (if attached) and any local `ufw`/`iptables` rules on the box — a one-time manual step, same as port 80 was for prod.
